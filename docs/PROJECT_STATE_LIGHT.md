...
> Last updated: 2026-05-15 (Sprint AX local - ResearchCase Operational View v1)
...
- Sprint AK: Kanban Usability, Source Query Fixes & Agent Ops Cleanup implemented locally. Kanban cards are cleaner, Hide Empty Phases works across board views, filters cover search, filing type, special situation type, and workflow phase using already-loaded metadata, horizontal navigation is clearer, Copy query has scoped copied/failure feedback, Intelligence Score explains score improvement steps, and Agent Ops now emphasizes workload/next manual actions instead of a large scheduler panel. Frontend/docs only; no backend automation, cron, scheduler execution, AI, evaluator, scanner, `/scan`, automatic evaluation, automatic ResearchCase creation, automatic promotion, crawling, document fetch, source verification, investment recommendations, publishing, or deploy.
- Sprint AL: Historical Analogues & Course Pattern Mapping implemented locally. New read-only endpoints map SpecialSituations and ResearchCases to processed playbook patterns and stored HistoricalCase metadata using deterministic filing type, situation type, playbook, required-resource, checklist, and methodology-status signals. Case detail pages show matched patterns, similar historical cases, differences, manual comparison checklist, warnings, and guardrails. Kanban cards show compact pattern labels, Intelligence KPIs links manual historical comparison context, and Agent Ops documents Pattern Analyst as observer/manual-only. Safe processed metadata only; no raw course materials/transcripts/excerpts, AI, evaluator, scanner, `/scan`, cron change, scheduler execution, automatic evaluation, automatic ResearchCase creation/promotion, crawling, document fetch, source verification, prediction, publishing, or deploy.
- Sprint AM: Case Completion Workbench & Manual Review Workflow implemented locally. New read-only endpoints convert existing case gaps into completion level, completion score, blocking items, next manual actions, score improvement plan, and section status. Situation and ResearchCase detail pages show the workbench near the top, Kanban cards show compact completion status from already-loaded JSON, Intelligence Score links to the completion plan, and Agent Ops documents Case Completion Coach as manual/observer-only. Deterministic/read-only only; no automatic completion, DB writes, AI, evaluator, scanner, `/scan`, cron, scheduler execution, automatic evaluation, ResearchCase creation/promotion, crawling, document fetch, source verification, publishing, or deploy.
- Sprint AN+AO: Product Surface Audit & SEC EDGAR Document Acquisition v1 implemented locally. Mission Control now groups the app around the core operating flow and de-emphasizes advanced/legacy/paused surfaces without deleting them. New manual SEC-only acquisition endpoints preview and explicitly acquire SEC document metadata/candidates for SpecialSituations and ResearchCases. SpecialSituation acquisition writes only `methodology_workspace.resource_candidates`; ResearchCase acquisition writes only `ResearchDocument`/`ResearchSource` metadata. Evidence remains unverified, checklist/resources are not completed automatically, and no ResearchCase creation, promotion, evaluation, publishing, AI, evaluator, scanner, `/scan`, cron change, scheduler execution, non-SEC fetch, source verification, or deploy was performed.
- Sprint AP: Executive Office & Improvement Proposals v1 implemented locally. Mission Control and Agent Ops now show Dani Weber Office (COO), Fontana Office (CTO), Executive Review, and Pending Improvement Proposals as deterministic, proposal-only governance surfaces. Agent Ops derives Executive Review rows and Improvement Proposals v1 from already-loaded operational rows and product-model context. Existing Agent Ops proposal status review remains the only write behavior; no backend endpoint, DB migration, live AI, evaluator, scanner, `/scan`, cron, scheduler execution, autonomous changes, verification, checklist completion, publishing, Marketplace/Sales change, public-site implementation, or deploy was added.
- Sprint AT: Review Polish & Executive Proposal Hardening implemented locally. Refreshed situation data after manual SEC acquisition, added SEC acquisition endpoints to deployment checks, hardened AI/evaluator guardrail tests, and made Agent Ops proposal type mapping deterministic. No backend/DB behavior change, automation, AI, evaluator, scanner, `/scan`, cron, deploy, or staging.
- Sprint AU: SEC Acquisition Deployment QA & Smoke-Test Polish implemented locally. Added deployment QA guidance and compact SEC Acquisition v1 smoke checks for manual preview/acquisition behavior, unchanged evidence/checklist status, and rollback notes. Documentation/UI guidance only; no backend behavior change, AI, evaluator, scanner, `/scan`, cron, deploy, or staging.
- Sprint AV: Dani Weber COO Metrics v1 implemented locally. Added deterministic read-only COO metrics for process/funnel bottlenecks, documentation blockers, source coverage, noise, and approval-required process improvements. No DB writes, auto-apply behavior, AI, evaluator, scanner, `/scan`, cron, deploy, or staging.
- Sprint AW: Fontana + Dani Weber Executive Review v1 implemented locally. Added deterministic read-only Executive Review that combines COO bottlenecks with CTO/product interpretation into approval-required joint findings, recommendations, and sprint candidates. No DB writes, auto-apply behavior, AI, evaluator, scanner, `/scan`, cron, deploy, or staging.
- Sprint AX: ResearchCase Operational View v1 implemented locally. Added deterministic read-only ResearchCase workflow-label preparation endpoint and detail-page card for Candidate / Watchlist / Reject / Insufficient Information, with manual-review guardrails and no automatic status changes. No DB writes, auto-application, AI, evaluator, scanner, `/scan`, cron, deploy, or staging.
...
2. Claude review of the current local multi-sprint batch through Sprint AX before deployment.
