...
2026-05-15 (Sprint AX local - ResearchCase Operational View v1)
...
- Sprint AK: Kanban Usability, Source Query Fixes & Agent Ops Cleanup implemented locally. Kanban Hide Empty Phases now works in the default pipeline and compact overview, cards are simplified with optional details, filters cover search, filing type, special situation type, and workflow phase using already-loaded data, and horizontal board navigation is clearer. Manual query copy now uses scoped clipboard feedback and fallback failure messaging. The Intelligence Score card explains how to improve preparation quality without changing scores automatically. Agent Ops replaces the large Scheduler/Frequency panel with workload and next manual actions, and long agent names/labels wrap safely. Frontend/docs only: no backend automation, cron, scheduler execution, AI, evaluator, scanner, `/scan`, automatic evaluation, automatic ResearchCase creation, automatic promotion, crawling, document fetch, source verification, investment recommendations, publishing, or deploy.
- Sprint AL: Historical Analogues & Course Pattern Mapping implemented locally. New deterministic read-only endpoints `GET /api/investment/situations/{id}/historical-analogues` and `GET /api/investment/research-cases/{id}/historical-analogues` map cases to processed course/playbook patterns and stored HistoricalCase metadata using filing type, situation type, playbook labels, required resources, checklist concepts, and methodology status. Situation and ResearchCase detail pages show matched patterns, similar historical cases, why they are similar, differences, manual comparison checklist, warnings, and guardrails. Kanban cards show compact pattern labels from already-loaded metadata, Intelligence KPIs links manual comparison context, and Agent Ops documents Pattern Analyst as manual/observer-only. It uses safe processed metadata only and does not include raw course materials, raw transcripts, copyrighted excerpts, AI, evaluator calls, scanner calls, `/scan`, cron changes, scheduler execution, automatic evaluation, ResearchCase creation, promotion, crawling, document fetching, source verification, prediction, publishing, or deploy.
- Sprint AM: Case Completion Workbench & Manual Review Workflow implemented locally. New deterministic read-only endpoints `GET /api/investment/situations/{id}/completion-workbench` and `GET /api/investment/research-cases/{id}/completion-workbench` convert stored case gaps into manual blocking items, next actions, section status, completion level, completion score, and score-improvement plans. Situation and ResearchCase detail pages show the workbench near the top; Kanban cards show compact completion status from already-loaded JSON; the Intelligence Score card links to the completion plan; and Agent Ops documents Case Completion Coach as manual/observer-only. No automatic completion, DB writes, live AI, evaluator, scanner, `/scan`, cron, scheduler execution, automatic evaluation, automatic ResearchCase creation, automatic promotion, crawling, document fetching, source verification, publishing, or deploy.
- Sprint AN+AO: Product Surface Audit & SEC EDGAR Document Acquisition v1 implemented locally. Mission Control now presents the core operating flow from SEC EDGAR cron through SpecialSituation triage, evidence completion, manual ResearchCase promotion, preparation quality, Dani manual decision, and manual publishing. Supporting, advanced, legacy, and paused surfaces are grouped below the primary workflow. New manual SEC-only endpoints preview and explicitly acquire SEC document metadata/candidates for SpecialSituations and ResearchCases. SpecialSituation acquisition writes only `methodology_workspace.resource_candidates`; ResearchCase acquisition writes only `ResearchDocument`/`ResearchSource` metadata. Evidence remains unverified, checklist/resources are not completed automatically, and no ResearchCase creation, promotion, evaluation, publishing, AI, evaluator, scanner, `/scan`, cron change, scheduler execution, non-SEC fetch, source verification, or deploy was performed.
- Sprint AP: Executive Office & Improvement Proposals v1 implemented locally. Mission Control and Agent Ops now expose Dani Weber Office (COO), Fontana Office (CTO), Executive Review, and Pending Improvement Proposals as deterministic, proposal-only governance surfaces. Agent Ops derives COO finding categories, CTO interpretations, joint next steps, guardrail notes, and Improvement Proposals v1 from already-loaded rows and static product-model context. Existing Agent Ops proposal status review remains the only write behavior; no new endpoint, service, DB migration, autonomous agent execution, deployment, scanner/evaluator call, cron change, ResearchCase automation, source/evidence verification, checklist completion, publishing, AI, Marketplace/Sales change, or public-site implementation was added.
- Sprint AT: Review Polish & Executive Proposal Hardening implemented locally. Situation SEC acquisition refreshes local data after manual action, SEC endpoints were added to deployment checks, AI/evaluator guardrail tests were hardened, and Agent Ops proposal type mapping is deterministic. No backend/DB behavior change, deploy, automation, AI, cron, scanner, evaluator, publishing, or staging.
- Sprint AU: SEC Acquisition Deployment QA & Smoke-Test Polish implemented locally. Added deployment QA guidance and compact SEC Acquisition v1 smoke checks covering manual preview/acquisition behavior, unchanged evidence/checklist status, and rollback notes. Documentation/UI guidance only; no backend behavior change, deploy, migration, scanner, cron, evaluator, AI, or staging.
- Sprint AV: Dani Weber COO Metrics v1 implemented locally. Added deterministic read-only COO metrics endpoint and Agent Ops integration for funnel/process bottlenecks, documentation blockers, source coverage, noise, and approval-required process improvements. No DB writes, auto-apply behavior, scanner, evaluator, AI, cron, deploy, or staging.
- Sprint AW: Fontana + Dani Weber Executive Review v1 implemented locally. Added deterministic read-only Executive Review combining COO bottlenecks with CTO/product interpretation into approval-required joint findings, recommendations, and sprint candidates. No DB writes, auto-apply behavior, scanner, evaluator, AI, cron, deploy, or staging.
- Sprint AX: ResearchCase Operational View v1 implemented locally. Added deterministic read-only ResearchCase workflow-label preparation endpoint and detail-page card for Candidate / Watchlist / Reject / Insufficient Information, with manual-review guardrails and no automatic status changes. No DB writes, auto-application, scanner, evaluator, AI, cron, deploy, or staging.
...
Next steps: Claude review of the current local multi-sprint batch through Sprint AX, then Dani manual deploy only if approved. Any future manual evaluation preview or evaluator v2 preview remains a separate explicit-approval gate without live AI/global evaluator activation by default.
